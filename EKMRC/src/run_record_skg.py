...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            model.train()
            # batch = tuple(t.to(args.device) for t in batch) # to device operation in model.py
            inputs = {'input_ids':       batch[0], 
                      'attention_mask':  batch[1],
                      'start_positions': batch[3],
                      'end_positions':   batch[4],
                      'core_entity_ids': batch[7],
                      'graph_data_dict': batch[8]}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {'input_ids':      batch[0],
                      'attention_mask': batch[1],
                      'core_entity_ids': batch[6],
                      'graph_data_dict': batch[7]
                      }
            if args.model_type != 'distilbert':
...
def main():
    ...
    # model params
    parser.add_argument("--gnn_layers", default=1, type=int, help="gnn layers")
    ...
    if args.do_train:
        train_dataset, entity_emb_mat, relation_emb_mat = load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False)
    else:
        _, entity_emb_mat, relation_emb_mat = load_and_cache_examples(args, tokenizer, evaluate=True, output_examples=False)
    ...
    model = model_class.from_pretrained(args.model_name_or_path,
                                        ...
                                        args=args,
                                        entity_emb_mat=entity_emb_mat,
                                        relation_emb_mat=relation_emb_mat)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = model_class.from_pretrained(args.output_dir, device=args.device, args=args,entity_emb_mat=entity_emb_mat, relation_emb_mat=relation_emb_mat)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint, device=args.device, args=args, entity_emb_mat=entity_emb_mat, relation_emb_mat=relation_emb_mat)
            model.to(args.device)
