...
from transformers import (WEIGHTS_NAME, BertConfig,
                                  KTNetForQuestionAnswering_both,
...
from transformers import AdamW, WarmupLinearSchedule, WarmupCosineSchedule

from utils_record_concept import (read_record_examples, convert_examples_to_features_both, read_squad_examples,
...
MODEL_CLASSES = {
    'ktnet': (BertConfig, KTNetForQuestionAnswering_both, BertTokenizer),
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids':       batch[0], 
                      ...
                      'wn_concept_ids': batch[7],
                      'nell_concept_ids': batch[8]}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {'input_ids':      batch[0],
                      'attention_mask': batch[1],
                      'wn_concept_ids': batch[6],
                      'nell_concept_ids': batch[7]
                      }
            if args.model_type != 'distilbert':
    ...
    results = evaluate_on_record(evaluate_options)
    
    # TODO: Evaluate with the official ReCoRD script
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    # load concept embedding mat(here both of wordnet and nell must be used)
    assert (args.use_wordnet or args.use_nell) is True
    if args.use_wordnet:
        logger.info("load wordnet embeddings....")
        wn_id2concept, wn_concept2id, wn_embedding_mat = read_concept_embedding("EKMRC/data/retrived_kb_data/kb_embeddings/wn_concept2vec.txt")
    if args.use_nell:
        logger.info("load nell embeddings....")
        nell_id2concept, nell_concept2id, nell_embedding_mat = read_concept_embedding("EKMRC/data/retrived_kb_data/kb_embeddings/nell_concept2vec.txt")
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        features = convert_examples_to_features_both(examples=examples,
                                                tokenizer=tokenizer,
                                                ...
                                                retrieved_nell_concept_path=args.retrieved_nell_concept_path,
                                                retrieved_wordnet_concept_path=args.retrieved_wordnet_concept_path,
    ...
    ## concept ids here
    all_wn_concept_ids = torch.tensor([f.wn_concept_ids for f in features], dtype=torch.long)
    all_nell_concept_ids = torch.tensor([f.nell_concept_ids for f in features], dtype=torch.long)
    if evaluate:
        all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_example_index, all_cls_index, all_p_mask, all_wn_concept_ids, all_nell_concept_ids)
    else:
        all_start_positions = torch.tensor([f.start_position for f in features], dtype=torch.long)
        all_end_positions = torch.tensor([f.end_position for f in features], dtype=torch.long)
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_start_positions, all_end_positions,
                                all_cls_index, all_p_mask, all_wn_concept_ids, all_nell_concept_ids)

    if output_examples:
        return dataset, examples, features
    return dataset, wn_embedding_mat, nell_embedding_mat
...
def main():
    ...
    parser.add_argument("--use_wordnet", action='store_true', default=True, help="if use wordnet")
    parser.add_argument("--use_nell", action='store_true', default=True, help="is use nell")
    ...
    if args.do_train:
        train_dataset, wn_concept_embedding_mat, nell_concept_embedding_mat = load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False)
    else:
        _, wn_concept_embedding_mat, nell_concept_embedding_mat = load_and_cache_examples(args, tokenizer, evaluate=True, output_examples=False)
    ...
    # Training
    if args.do_train:
        # train_dataset, concept_embedding_mat = load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = model_class.from_pretrained(args.output_dir, wn_concept_embedding_mat=wn_concept_embedding_mat,
                                        nell_concept_embedding_mat=nell_concept_embedding_mat)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint, wn_concept_embedding_mat=wn_concept_embedding_mat,
                                        nell_concept_embedding_mat=nell_concept_embedding_mat)
            model.to(args.device)

            logger.info("Evaluating......")
            logger.info("Evaluating models {}".format(model))
