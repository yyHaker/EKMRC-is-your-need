...
from transformers import (WEIGHTS_NAME, BertConfig,
                                  FSNet_ner_ForQuestionAnswering,
...
from utils_record_concept_ner import (read_record_examples, convert_examples_to_features,
...
MODEL_CLASSES = {
    'fsnet_ner': (BertConfig, FSNet_ner_ForQuestionAnswering, BertTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    args.loggin_steps = t_total // args.num_train_epochs
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if args.use_wordnet:
        ...
        id2concept, concept2id, embedding_mat = read_concept_embedding("EKMRC/kgs/wordnet/wordnet_emb/wn_concept2vec.txt")
    elif args.use_nell:
        logger.info("load nell embeddings....")
        id2concept, concept2id, embedding_mat = read_concept_embedding("EKMRC/kgs/NELL/nell_emb/nell_concept2vec.txt")

    # Load data features from cache or dataset file
    input_file = args.predict_file if evaluate else args.train_file
    cpnet_name = "wordnet" if args.use_wordnet else "nell"
    ...
    if evaluate:
        ...
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_example_index, all_cls_index, all_p_mask, all_concept_ids, all_concept_masks, all_label_ids)
    else:
        all_start_positions = torch.tensor([f.start_position for f in features], dtype=torch.long)
        all_end_positions = torch.tensor([f.end_position for f in features], dtype=torch.long)
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_start_positions, all_end_positions,
                                all_cls_index, all_p_mask, all_concept_ids, all_concept_masks, all_label_ids)

    if output_examples:
        return dataset, examples, features
    return dataset, embedding_mat
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--train_file", default="EKMRC/data/ReCoRD/ReCoRD_debug/train.json", type=str,
                        help="ReCoRD json for training. E.g., train.json")
    parser.add_argument("--predict_file", default="EKMRC/data/ReCoRD/ReCoRD_debug/dev.json", type=str,
                        help="ReCoRD json for predictions. E.g., dev.json or test.json")
    parser.add_argument("--model_type", default="fsnet_ner", type=str,
                        help="Model type selected in the list: " + ", ".join(MODEL_CLASSES.keys()))
    parser.add_argument("--model_name_or_path", default="/users4/yle/pretrained_models/bert-large-uncased-whole-word-masking", type=str,
    ...
    if args.do_train:
        ...
        _, concept_embedding_mat = load_and_cache_examples(args, tokenizer, evaluate=True, output_examples=False)
